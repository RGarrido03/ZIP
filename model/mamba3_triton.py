...
class Mamba3(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_ncssd = use_ncssd
        self.mimo_rank = mimo_rank
        if not self.is_mimo:
            self.mimo_rank = 1

        self.d_inner = int(self.expand * self.d_model)
        # Auto-adjust headdim to the largest divisor of d_inner not exceeding the requested headdim.
        # This gracefully handles configs where d_inner isn't divisible by the fixed headdim
        # (e.g. embed_dims=[48, 64, 168, 224] with headdim=48 — d_model=64 → d_inner=128 not divisible by 48).
        if self.d_inner % self.headdim != 0:
            for hd in range(min(self.headdim, self.d_inner), 0, -1):
                if self.d_inner % hd == 0:
                    print(
                        f"  Adjusted headdim from {self.headdim} → {hd} "
                        f"(d_inner={self.d_inner}, d_model={d_model})"
                    )
                    self.headdim = hd
                    break
        print(f"Initializing Mamba3 with d_inner={self.d_inner}, d_model={d_model}, d_state={d_state}, expand={expand}, headdim={self.headdim}, num_heads={self.d_inner // self.headdim}, mimo_rank={self.mimo_rank}, use_ncssd={use_ncssd}")
        ...
        # NC-SSD: Non-Causal State Space Duality (matrix-form attention)
        if self.use_ncssd:
            self.ncssd = NCSSD_Mamba3(
                nheads=self.nheads,
                num_bc_heads=self.num_bc_heads,
                headdim=self.headdim,
                d_state=self.d_state,
                split_tensor_size=self.split_tensor_size,
                num_rope_angles=self.num_rope_angles,
                mimo_rank=self.mimo_rank,
                is_mimo=self.is_mimo,
                is_outproj_norm=self.is_outproj_norm,
                norm=self.norm if self.is_outproj_norm else None,
            )


    def forward(self, u, seq_idx=None, cu_seqlens=None, inference_params=None, external_angles=None):
        """
        u: (batch, seqlen, hidden_dim)
        external_angles: (B, L, N, S) optional pre-computed 2D spatial RoPE angles
            for vision tasks.  When provided, overrides the 1D content-dependent
            angles from in_proj.  Shape: (batch, seqlen, nheads, num_rope_angles).
        ...
        if external_angles is not None:
            angles = external_angles.to(torch.float32)  # (B, L, N, S) — pre-computed 2D RoPE
        else:
        ...
        # Apply NC-SSD or Mamba-3 kernel
        if self.is_mimo:
            y = self.ncssd(x, z, B, C, ADT, trap, angles, self.D,
